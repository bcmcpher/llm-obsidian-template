...
This skill parses `_meta/log.md` to answer activity and status questions about the vault. It does not modify the log, does not scan source files directly (except for cross-reference checks), and does not generate synthesis reports (that belongs to `karpathy-wiki-compose`).
...
| "Show reconcile/refactor sessions" | `skill::` = `karpathy-wiki-reconcile` or `karpathy-wiki-refactor` |
...
Log entries involving [[transformer-architecture]]: 2
  2026-04-27  paper  | Attention Is All You Need    (skill: karpathy-wiki-ingest)
  2026-04-20  video  | Andrej Karpathy GPT lecture  (skill: karpathy-wiki-connect)
```
...
⚠ Status inconsistencies found: 1
  sources/paper/2026-04-27-attention-is-all-you-need.md
    Log says processed on 2026-04-27 (karpathy-wiki-ingest)
...
- If the log is empty overall, note: "The log has no entries yet. Entries are written by karpathy-wiki-ingest, karpathy-wiki-connect, and other processing skills."
...
- Does not generate synthesis reports — use `karpathy-wiki-compose` for that
