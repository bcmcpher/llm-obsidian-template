...
- After a split or merge, suggest running `karpathy-wiki-trust-audit` on the affected topic: confidence: low on new atoms is expected but should be revisited once sources are re-evaluated
- After a split or merge, suggest running `karpathy-wiki-reconcile` to catch any `part-of` / `covers::` drift introduced by the re-pointing
