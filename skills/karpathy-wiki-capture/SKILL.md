...
This skill does one thing: get a URL into the vault as fast as possible. Metadata enrichment, summaries, and connection wiring all happen later in `karpathy-wiki-connect`. The only thing that can't be recovered later is *why* the user saved it — so that's the only thing worth asking.
...
## Summary
<!-- To be filled by karpathy-wiki-connect -->
...
### 5. Confirm and close
Report the file path. One line: note is in the inbox, run `karpathy-wiki-connect` to enrich and wire it.
...
- Don't create a note if the URL is already in `sources/` — do a quick check first:
  `grep -rl "<url>" /home/bcmcpher/Projects/claude/karpathy-wiki/sources/`
