---
name: karpathy-wiki-trust-audit
...
url:: n/a
atoms:: [[Atom A]], [[Atom B]]
skill:: karpathy-wiki-trust-audit
...
- Does not reprocess sources or add new connections — that belongs to `karpathy-wiki-connect`
...
- After `karpathy-wiki-reconcile` (graph must be consistent before evaluating confidence)
- Before running `karpathy-wiki-compose` (export quality depends on trustworthy confidence signals)
...
- Don't audit an atom that hasn't been through `karpathy-wiki-reconcile` — a disconnected graph produces misleading provenance counts
